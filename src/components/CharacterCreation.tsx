...
// Calculate base score according to V2.0 rules
function calculateBaseScore(attributes: Attribute[]): { 
  total: number, 
  potentialSum: number,
  modifiers: Modifier[], 
  synergies: Modifier[] 
  ...
  return {
    total: potentialSum + modifierTotal + synergyTotal,
    potentialSum,
    modifiers,
    synergies
  };
...
export default function CharacterCreation({ language, archetype, onComplete }: CharacterCreationProps) {
  ...
  const handleGenerate = () => {
    ...
    // Calculate base score with V2.0 rules
    const breakdown = calculateBaseScore(attributes);
  ...
  if (generated && scoreBreakdown) {
    ...
    return (
      <div className={heroBg}>
        <section className="pt-24 sm:pt-32 pb-16 sm:pb-24 px-4 sm:px-8">
  ...
  return (
    <div className={heroBg}>
      <section className="pt-24 sm:pt-32 pb-4 sm:pb-6 px-4 sm:px-8">
        <div className="max-w-5xl mx-auto">
          {/* Header */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
