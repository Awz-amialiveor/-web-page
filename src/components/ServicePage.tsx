...
export default function ServicePage({ language, onNavigate, archetype }: ServicePageProps) {
  const t = translations[language];

  const heroBg = archetype
    ? "bg-gradient-to-b from-[var(--theme-from)] to-[var(--theme-via)]"
    : "bg-black";

  return (
    <div className="bg-black">
      {/* Hero */}
      <section className={`pt-24 sm:pt-32 pb-16 sm:pb-24 px-4 sm:px-6 md:px-8 ${heroBg} relative overflow-hidden`}>
        {/* Geometric Circles Background */}
        <div className="absolute top-0 left-10 sm:left-20 w-48 h-48 sm:w-80 sm:h-80 rounded-full border-[20px] sm:border-[40px] border-white/5 z-0"></div>
        <div className="absolute bottom-0 right-5 sm:right-10 w-32 h-32 sm:w-64 sm:h-64 rounded-full border-[15px] sm:border-[30px] border-red-600/10 z-0"></div>
        
        <div className="max-w-6xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-[0.6rem] sm:text-xs tracking-[0.3em] sm:tracking-[0.5em] text-white/40 mb-6 sm:mb-8">III. SERVICES</p>
            ...
            <div className="flex justify-center mb-6 sm:mb-8">
              <div className="w-8 sm:w-12 h-1 bg-red-600"></div>
            </div>
            <p className="text-lg sm:text-xl md:text-2xl text-white/60 mb-4 sm:mb-6 px-4" style={{ fontWeight: 300 }}>{t.heroSubtitle}</p>
            <p className="text-base sm:text-lg text-white/50 px-4">{t.heroDescription}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-32 px-4 sm:px-6 md:px-8 bg-black">
