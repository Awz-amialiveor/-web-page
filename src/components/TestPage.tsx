...
import badgesImage from 'figma:asset/e6047704a049d53e8f3d3b143fe50bca45a0d6f5.png';
...
const archetypes: Archetype[] = [
  ...
  },
  {
    ...
    descriptionZh: '对你而言，生命是一项需要被精心规划、执行和完成的工程。混乱与随机让你感到不安，而秩序、逻辑与成就则能给你带来最深的满足感。你的每一个决定都是一张蓝图，每一个成就都是一块基石。你为自己建造的这座人生丰碑感到骄傲，并希望它的每一个细节都能被准确地理解和记录。你的告别，不应是一场随意的落幕，而应是你一生事业的最终、也是最完美的总结陈词，是你为自己这座建筑举行的、庄重的封顶仪式。',
...
const questions: Question[] = [
  ...
  },
  {
    ...
    options: [
      ...
      { label: 'D', textEn: 'A sculpture of extraordinary taste', textZh: '一件品味不凡的塑' }
    ]
  },
  {
    id: 8,
...
export default function TestPage({ language, onArchetypeSet }: TestPageProps) {
  ...
  if (!started) {
    return (
      <div className="bg-gradient-to-b from-black via-zinc-900 to-zinc-800">
        <section className="min-h-screen pt-32 pb-24 px-8 relative overflow-hidden flex items-center justify-center">
          <div className="absolute inset-0 z-0 opacity-5">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full border-[40px] border-white"></div>
          </div>

          <div className="max-w-4xl mx-auto text-center relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
              <p className="text-xs tracking-[0.5em] text-white/40 mb-8">III. CALIBRATION</p>
              ...
              <p className="text-2xl text-white/60 mb-6" style={{ fontWeight: 300 }}>
                {t.subtitle}
              </p>
              <p className="text-base text-white/50 mb-16 max-w-2xl mx-auto leading-relaxed">
                {t.description}
              </p>
              ...
              <motion.button
                ...
                className="px-20 py-7 bg-red-600 text-white hover:bg-red-700 transition-all text-xs tracking-[0.3em]"
              >
                {t.startButton}
              </motion.button>
  ...
  if (completed && result) {
    ...
    // Badge positions in the sprite (4 badges in 2x2 grid)
    const badgePositions = {
      'A': { x: 0, y: 0 },      // Archive Keeper - top left
      'B': { x: 50, y: 0 },     // Future Explorer - top right
      'C': { x: 0, y: 50 },     // Life Architect - bottom left
      'D': { x: 50, y: 50 }     // Romantic Poet - bottom right
    };
    
    const badgePos = badgePositions[result.key];
    
    return (
      <div className="bg-gradient-to-b from-[var(--theme-from)] via-[var(--theme-via)] to-white">
        <section className="pt-32 pb-24 px-8">
          <div className="max-w-5xl mx-auto">
            ...
            >
              <p className="text-xs tracking-[0.5em] text-white/60 mb-8">{t.resultTitle}</p>
              <div className="flex justify-center mb-8">
                <div className="w-16 h-[1px] bg-white/30"></div>
              </div>
              ...
              >
                <div 
                  className="w-48 h-48 mx-auto rounded-full overflow-hidden border-4 border-white/20 shadow-2xl"
                  style={{
                    backgroundImage: `url(${badgesImage})`,
                    backgroundSize: '200% 200%',
                    backgroundPosition: `${badgePos.x}% ${badgePos.y}%`
                  }}
                />
              </motion.div>
            </motion.div>
            ...
            <motion.div
              ...
              className="bg-white p-16 mb-12"
            >
              <div className="flex items-center gap-8 mb-12 pb-12 border-b border-black/10">
                <div className="bg-black p-8">
                  <Icon className="w-16 h-16 text-white" />
                </div>
                <div className="flex-1">
                  <p className="text-xs tracking-[0.3em] text-black/40 mb-3">{t.archetype}</p>
                  <h2 className="text-5xl text-black mb-2" style={{ fontWeight: 300 }}>
                    {language === 'en' ? result.nameEn : result.nameZh}
                  </h2>
                  <div className="flex gap-3 mt-4">
                    {(language === 'en' ? result.keywordsEn : result.keywordsZh).map((keyword, idx) => (
                      <span key={idx} className="px-4 py-2 bg-black text-white text-xs tracking-wider">
                        {keyword}
                      </span>
                    ))}
                  </div>
              ...
              {/* Quote */}
              <div className="mb-12">
                <p className="text-xs tracking-[0.3em] text-black/40 mb-4">{t.quote}</p>
                <p className="text-2xl text-black/80 italic leading-relaxed" style={{ fontWeight: 300 }}>
              ...
              {/* Description */}
              <div className="mb-12">
                <p className="text-xs tracking-[0.3em] text-black/40 mb-4">{t.description}</p>
              ...
              {/* Core Desire */}
              <div className="mb-12 bg-black/5 p-8">
                <p className="text-xs tracking-[0.3em] text-black/40 mb-4">{t.coreDesire}</p>
                <p className="text-xl text-black leading-relaxed" style={{ fontWeight: 300 }}>
              ...
              <div className="bg-black text-white p-10">
                <div className="flex items-center gap-4 mb-6">
                  <p className="text-xs tracking-[0.3em] text-white/60">{t.recommendation}</p>
                  <div className="flex-1 h-[1px] bg-white/20"></div>
                  <p className="text-xl tracking-wider" style={{ fontWeight: 300 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <button
                ...
                className="px-12 py-5 bg-white text-black border border-white hover:bg-zinc-100 transition-all text-xs tracking-[0.3em]"
              >
                {t.retake}
              </button>
              <button className="px-12 py-5 bg-white text-black hover:bg-zinc-100 transition-all text-xs tracking-[0.3em]">
                {t.scheduleConsult}
              </button>
            </div>
          </div>
  ...
  return (
    <div className="bg-gradient-to-b from-black via-zinc-900 to-zinc-800 min-h-screen">
      {/* Progress Bar */}
      <div className="fixed top-20 left-0 right-0 z-40 bg-black/80 backdrop-blur-lg border-b border-white/10">
        <div className="px-8 py-4">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-3">
              ...
              <p className="text-xs tracking-[0.3em] text-white/60">{Math.round(progress)}%</p>
            </div>
        ...
        <div className="max-w-4xl mx-auto">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Question */}
              <div className="mb-12">
                <p className="text-xs tracking-[0.3em] text-white/40 mb-6">{String(question.id).padStart(2, '0')}</p>
                <h2 className="text-4xl text-white mb-6 leading-tight" style={{ fontWeight: 300 }}>
              ...
              <div className="space-y-3 mb-12">
                {question.options.map((option) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.99 }}
                    className={`w-full p-5 text-left transition-all border-2 ${
              ...
              <div className="flex items-center justify-between">
                <button
                  ...
                  className="flex items-center gap-3 px-8 py-4 bg-white/10 text-white border border-white/20 hover:bg-white/20 transition-all text-xs tracking-[0.2em] disabled:opacity-30 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-3 px-8 py-4 bg-white text-black hover:bg-zinc-100 transition-all text-xs tracking-[0.2em] disabled:opacity-30 disabled:cursor-not-allowed"
                >
