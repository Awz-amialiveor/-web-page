...
type Language = 'en' | 'zh';
type PageType = 'main' | 'service' | 'calculation' | 'test' | 'community';
...
export default function Navigation({ currentPage, onPageChange, language, onLanguageChange, archetype }: NavigationProps) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 ${bgColor} border-b border-white/10`}>
      <div className="w-full mx-auto px-4 sm:px-6 md:px-8 py-4 sm:py-5 flex items-center justify-between">
        {/* Left: Logo */}
        <button 
        ...
        <div className="hidden lg:flex items-center gap-1">
          {(['main', 'service', 'calculation', 'test', 'community'] as PageType[]).map((page) => (
            <button
              key={page}
