...
import { Baby, Brain, Users, Briefcase, AlertTriangle, ChevronDown, ChevronUp, Crown, Award, Star, User, Skull } from 'lucide-react';
...
export default function CalculationPage({ language }: CalculationPageProps) {
  ...
  return (
    <div className="bg-gradient-to-b from-black via-zinc-900 to-white">
      {/* Hero Section */}
      <section className="pt-32 pb-24 px-8 bg-gradient-to-b from-black via-zinc-900 to-zinc-800 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-xs tracking-[0.5em] text-white/40 mb-8">II. CALCULATION</p>
            ...
            <p className="text-2xl text-white/60 mb-6" style={{ fontWeight: 300 }}>{t.subtitle}</p>
            <p className="text-lg text-white/50">{t.description}</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-5 gap-8 items-center">
              <div className="md:col-span-2 text-center md:text-left">
                <p className="text-xs tracking-[0.3em] text-white/40 mb-3">{t.yourScore}</p>
                <motion.div
                  key={totalScore}
                ...
                >
                  <span className="text-[5rem] text-white leading-none" style={{ fontWeight: 300 }}>
                    {totalScore}
                  </span>
                  <span className="text-lg text-white/50 tracking-[0.2em]">{t.points}</span>
                </motion.div>
              </div>
              ...
              <motion.div
                ...
                animate={{ scale: 1, opacity: 1 }}
                className={`md:col-span-3 ${tier.bgColor} border-2 ${tier.borderColor} p-8 relative overflow-hidden`}
              >
                <div className="absolute inset-0 opacity-5">
                  <div className="absolute top-0 right-0 w-32 h-32 rounded-full bg-white"></div>
                </div>
                <div className="relative z-10 flex items-center gap-6">
                  <div className={`p-4 border ${tier.borderColor}`}>
                    <TierIcon className={`w-10 h-10 ${tier.textColor}`} />
                  </div>
                  <div className="flex-1">
                    <p className={`text-xs tracking-[0.3em] ${tier.textColor} opacity-60 mb-2`}>{t.tier}</p>
                    <p className={`text-4xl mb-2 ${tier.textColor} tracking-wider`} style={{ fontWeight: 300 }}>
                      {tier.name}
                    </p>
                    <p className={`text-sm ${tier.textColor} opacity-70`}>{tier.desc}</p>
                  </div>
                </div>
              </motion.div>
            </div>
      ...
      {/* Controls */}
      <div className="px-8 py-6 bg-white border-y border-black/10">
