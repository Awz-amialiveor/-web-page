...
import { Baby, Brain, Users, Briefcase, AlertTriangle, ChevronDown, ChevronUp, Crown, Award, Star, User, Skull, Eye, Shield, Heart as HeartIcon } from 'lucide-react';
...
const translations = {
  en: {
    ...
    legacyScore: 'Generational Legacy',
    legacyDesc: 'Inherited social standing (immutable)',
    base: 'Base',
    legacy: 'Legacy',
    bonus: 'Bonus'
  },
...
export default function CalculationPage({ language, archetype }: CalculationPageProps) {
  ...
  // Generational Legacy Score - random fixed value between -200 and +200
  const [legacyScore] = useState(() => Math.floor(Math.random() * 401) - 200);
  ...
  // Calculate total (base score + legacy score + bonus scores)
  useEffect(() => {
    const bonusTotal = Object.values(scores).reduce((sum, val) => sum + val, 0);
    setTotalScore(baseScore + legacyScore + bonusTotal);
  }, [scores, baseScore, legacyScore]);
  ...
  return (
    <div className={mainBg}>
      ...
      <section id="bonus-calculator" className={`pt-16 sm:pt-24 pb-16 sm:pb-24 px-4 sm:px-8 ${heroBg} relative overflow-hidden`}>
              ...
              <div className="md:col-span-2 text-center md:text-left">
                ...
                {baseScore > 0 && (
                  <div className="mt-2 sm:mt-3 flex flex-wrap gap-2 sm:gap-3 text-xs sm:text-sm text-white/60 justify-center md:justify-start">
                    ...
                    <span>+</span>
                    <span className={legacyScore >= 0 ? 'text-green-400' : 'text-red-400'}>
                      {t.legacy}: {legacyScore > 0 ? '+' : ''}{legacyScore}
                    </span>
                    <span>+</span>
      ...
      {/* Generational Legacy Score - Immutable */}
      <section className="py-12 px-8 bg-gradient-to-b from-white to-zinc-50 border-y border-black/10">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs tracking-[0.3em] text-black/40 mb-2">{language === 'en' ? 'IMMUTABLE' : '不可更改'}</p>
                  <h3 className="text-3xl text-black" style={{ fontWeight: 300 }}>
                    {t.legacyScore}
                  </h3>
                  <p className="text-sm text-black/60 mt-2">{t.legacyDesc}</p>
                </div>
              </div>
              <div className="text-right">
                <motion.p
                  ...
                  animate={{ scale: 1, opacity: 1 }}
                  className={`text-6xl ${legacyScore >= 0 ? 'text-green-600' : 'text-red-600'}`}
            ...
            <div className="bg-black/5 p-4 border-l-4 border-black">
              <p className="text-xs text-black/70 leading-relaxed">
                {language === 'en' 
                  ? 'This value represents your inherited social standing based on family background. It cannot be changed and reflects circumstances beyond your control.'
                  : '此数值代表你基于家庭背景继承的社会地位。它无法被改变，反映了你无法控制的环境因素。'
                }
              </p>
            </div>
          </motion.div>
      ...
      {/* Controls */}
      <div className="px-8 py-6 bg-white border-b border-black/10">
